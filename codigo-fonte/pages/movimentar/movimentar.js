...
let filtroTotalReceitas = filtroTotal.filter(transacao => transacao.idCategoria === "receitas")

// Condição para filtrar as transações de despesas do usuário logado
let filtroTotalDespesas = filtroTotal.filter(transacao => transacao.idCategoria === "despesas")

// Condição para filtrar as transações de poupancaIn do usuário logado
let filtroTotalPoupancaIn = filtroTotal.filter(transacao => transacao.idCategoria === "poupanca" && transacao.idTipo === "entrada")

// Condição para filtrar as transações de poupancaOut do usuário logado
let filtroTotalPoupancaOut = filtroTotal.filter(transacao => transacao.idCategoria === "poupanca" && transacao.idTipo === "saida")
